...
% Training or making predictions for test data (test)
...
predopt.train_local_sizes = [120*1.00, 120*0.25]; % train test sampling sizes
...
%% Additional feature generation for test dataset

if predopt.stage == "test"
    % Cyclic features related to-
    % for weekday (1~7)
    [test_day_sin, test_day_cos] = cyc_feat_transf(weekday(test_data.time),7);
    test_data.day_sin = test_day_sin;
    test_data.day_cos = test_day_cos;
    
    % for hour of day (0~23)
    [test_hour_sin, test_hour_cos] = cyc_feat_transf(hour(test_data.time),24);
    test_data.hour_sin = test_hour_sin;
    test_data.hour_cos = test_hour_cos;
end
...
switch predopt.stage
    case "training_local"
        ...
        switch predopt.sampling
            ...
            case "hybrid"
                % first, sort the table w.r.t. time
                tbl_all_sorted = sortrows(tbl_all,"time");
    ...
    case "test"
        switch predopt.sampling
            ...
            case "hybrid"
                ...
                % training from past
                train_past_idx_end = size(tbl_all_sorted,1); % since using the entire matrix
                train_past_idx_start = train_past_idx_end - train_size_past + 1;
                train_past_idxs = transpose(train_past_idx_start:1:train_past_idx_end);
                ...
                % the entire train+test dataset idxs
                idx_samp = vertcat(train_rand_idxs, train_past_idxs);
...
if predopt.stage == "training_local"
    switch predopt.sampling
...
switch predopt.stage
    case "training_local"
        ...
        if predopt.hyperparam == 1 % hyperparameter optimization
            % Which prediction method to use for GPR
            fit_pred_method = 'fic';
            if predopt.out_disp == 1
                disp("Running hyperopt...");
            end
            gprMdl = fitrgp(tbl_subset, 'pm2d5',...
                    'FitMethod', fit_pred_method, 'PredictMethod', fit_pred_method, 'Standardize', 1,...
            ...
            fit_pred_method = 'fic';
            if predopt.out_disp == 1
                disp("Running GPR training w/ cv...");
            end
            gprMdl = fitrgp(tbl_subset, 'pm2d5', ...
            ...
            y_pred_all = kfoldPredict(gprMdl); % this includes NaN!!!
            y_pred = y_pred_all(cvp.test,:);

            % code from making baseline model
    ...
    case "test"
        ...
        fit_pred_method = 'fic';
        if predopt.out_disp == 1
            disp("Running GPR training for test prediction...");
        end
        gprMdl = fitrgp(tbl_subset, 'pm2d5', ...
                'BasisFunction', 'linear',...
                'KernelFunction', 'ardmatern32',...
        ...
        subset_table_test = {'hmd', 'tmp', 'lat', 'lon', ...
            'day_sin', 'day_cos', 'hour_sin', 'hour_cos'};
        y_pred = predict(gprMdl,test_data(:,subset_table_test));
        save(strcat(predopt.mode,'_',num2str(predopt.var_level),'.mat'),'y_pred');
